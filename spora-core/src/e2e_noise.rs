...
impl NoiseHandshake {
    fn build(
        ...
        .map_err(|e| format!("noise handshake build: {e}"))?;
        Ok(Self { hs })
    }
    ...
    pub(crate) fn responder(identity: &Identity) -> Result<Self, String> {
        Self::build(&identity.routing_key, &identity.secret, false)
    }

    /// Write the next handshake message; returns the bytes to send.
    pub(crate) fn write_message(&mut self, payload: &[u8]) -> Result<Vec<u8>, String> {
    ...
    pub(crate) fn read_message(&mut self, msg: &[u8]) -> Result<Vec<u8>, String> {
    ...
    pub(crate) fn into_session(self) -> Result<NoiseSession, String> {
...
impl NoiseSession {
    ...
    pub(crate) fn encrypt(&self, counter: u64, plaintext: &[u8]) -> Result<Vec<u8>, String> {
    ...
    pub(crate) fn decrypt(&self, counter: u64, ciphertext: &[u8]) -> Result<Vec<u8>, String> {
...
pub(crate) fn build_cert_auth(
    ...
    .map_err(|e| format!("noise auth: rejected key: {e}"))?;
    ...
    let sig = key_pair
        .sign(&rng, &input)
        .map_err(|_| "noise auth: signing failed".to_string())?;
...
pub(crate) fn verify_cert_auth(
    ...
    if auth.len() < 2 {
        return Err("noise auth: truncated".into());
    }
    let cert_len = u16::from_be_bytes([auth[0], auth[1]]) as usize;
    if auth.len() < 2 + cert_len + 1 {
        return Err("noise auth: truncated cert/sig".into());
    }
    ...
    if digest.as_ref()[..ROUTING_KEY_LEN] != routing_key[..] {
        return Err("noise auth: routing key does not match cert".into());
    }
    ...
    eec.verify_signature(webpki::ring::ECDSA_P256_SHA256, &input, signature)
        .map_err(|_| "noise auth: bad signature".to_string())?;
    Ok(())
}
...
mod tests {
    ...
    fn handshake_pair(
        ...
        identity: &Identity,
    ) -> Result<(NoiseSession, NoiseSession), String> {
    ...
    fn cert_auth_rejects_a_routing_key_mismatch() {
        ...
        assert!(err.contains("routing key"), "{err}");
    }
